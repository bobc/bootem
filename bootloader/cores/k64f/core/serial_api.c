...
static uint32_t uart_addrs[] = UART_BASE_ADDRS;

static int stdio_uart_inited = 0;
static serial_t stdio_uart;


void serial_init(serial_t *obj, uint32_t port_num, tPinDef tx, tPinDef rx) {
...
//    pinmap_pinout(rx, PinMap_UART_RX);
    ...
    pin_function (tx.W, GPIO_FUNCTION(tx.W));
    pin_function (rx.W, GPIO_FUNCTION(rx.W));

//    pin_mode(tx.W, PullUp);
//    pin_mode(rx.W, PullUp);
...
int serial_readable(serial_t *obj) {
    if (UART_HAL_GetStatusFlag(uart_addrs[obj->index], kUartRxOverrun))
        UART_HAL_ClearStatusFlag(uart_addrs[obj->index], kUartRxOverrun);
    return UART_HAL_IsRxDataRegFull(uart_addrs[obj->index]);
}
...
int serial_writable(serial_t *obj) {
    ...
    return UART_HAL_IsTxDataRegEmpty(uart_addrs[obj->index]);
}
...
void serial_break_set(serial_t *obj) {
    UART_HAL_SetBreakCharCmd(uart_addrs[obj->index], true);
}

void serial_break_clear(serial_t *obj) {
    UART_HAL_SetBreakCharCmd(uart_addrs[obj->index], false);
}
